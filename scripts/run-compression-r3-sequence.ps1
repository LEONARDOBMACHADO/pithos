...
param(
    ...
    [string]$ExternalEvidenceRoot = 'C:\PithosStack\evidence-r3',
...
$allBranches = @(
    'perf/23-parallel-group-pack',
    'perf/24-parallel-native-read',
    'feat/25-global-canonical-pool',
    'feat/26-motif-codebook',
    'feat/27-implicit-global-references',
    'feat/28-quaternary-context',
    'perf/29-global-representation-planner'
)
...
$sessionRoot = Join-Path $ExternalEvidenceRoot ("r3-sequence-" + (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ'))
...
try {
    ...
    @(
        "requested_start_branch=$($branches[0])",
        "requested_branch_count=$expectedBranches",
        'campaign=REPRESENTATION_R3',
        'comparison_scope=pithos-archive-max-vs-7zip-mx9',
        'r2_size_floor_bytes=86100290',
        'winrar=INTENTIONALLY_NOT_USED',
        'winzip=INTENTIONALLY_NOT_USED'
    ) | Set-Content -LiteralPath (Join-Path $sessionRoot 'R3_SEQUENCE_SCOPE.txt') -Encoding UTF8
    ...
    foreach ($branch in $branches) {
        ...
        "Pithos representation R3 branch: $branch" | Set-Content -LiteralPath $currentLog -Encoding UTF8

        Invoke-NativeStep -Label "sync $branch to origin" -Log $currentLog -Action { & git switch -C $branch "origin/$branch" }
        ...
        Invoke-NativeStep -Label 'commit branch evidence' -Log $currentLog -Action { & git commit -m "test: benchmark representation R3 $branch against 7-Zip" }
        ...
        $sequenceRows.Add([pscustomobject]@{
            ...
            delta_bytes=($pithosArchiveBytes-$sevenArchiveBytes)
            delta_vs_r2_floor_bytes=($pithosArchiveBytes-86100290)
    ...
    $summaryCsv = Join-Path $sessionRoot 'R3_SEQUENCE_SUMMARY.csv'; $sequenceRows | Export-Csv -LiteralPath $summaryCsv -NoTypeInformation -Encoding UTF8
    $summaryTxt = Join-Path $sessionRoot 'R3_SEQUENCE_SUMMARY.txt'
    @(
        "completed_utc=$((Get-Date).ToUniversalTime().ToString('o'))",
        "start_branch=$($branches[0])",
        "branches_passed=$($sequenceRows.Count)",
        ...
        'winzip=INTENTIONALLY_NOT_USED',
        'status=PASS','',
        ($sequenceRows | Format-Table branch,pithos_archive_bytes,sevenzip_archive_bytes,delta_bytes,delta_vs_r2_floor_bytes,pithos_compress_ms,sevenzip_compress_ms,pithos_verify_ms,pithos_decompress_ms -AutoSize | Out-String)
    ) | Set-Content -LiteralPath $summaryTxt -Encoding UTF8
    Write-Host "`nALL $expectedBranches REPRESENTATION R3 BRANCHES PASSED" -ForegroundColor Green
    Write-Host "Summary: $summaryCsv"
    exit 0
}
catch {
    $failure = Join-Path $sessionRoot 'R3_SEQUENCE_FAILURE.txt'
    @(
        "failed_utc=$((Get-Date).ToUniversalTime().ToString('o'))",
        "requested_start_branch=$(if ($branches.Count -gt 0) { $branches[0] } else { '' })",
        "branch=$currentBranch",
        "source_sha=$currentSha",
        "log=$currentLog",
        "error=$($_.Exception.Message)",'',
        'STOP: later R3 branches were NOT tested.',
        'Do not modify source. Return this file and the branch log.'
    ) | Set-Content -LiteralPath $failure -Encoding UTF8
    Write-Error "R3 STACK STOPPED on $currentBranch : $($_.Exception.Message). Later branches were NOT tested. Evidence: $failure"
    exit 1
}

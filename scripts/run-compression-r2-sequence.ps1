...
param(
    ...
    [string]$ExternalEvidenceRoot = 'C:\PithosStack\evidence-r2',
...
$allBranches = @(
    'perf/16-archive-max-size-first',
    'feat/17-standard-content-reorder',
    'feat/18-lzma2-large-dict',
    'perf/19-lzma2-parallel',
    'perf/20-archive-max-finalist-race',
    'feat/21-dedup-lzma2-native',
    'feat/22-class-aware-solid-groups'
)
...
$sessionRoot = Join-Path $ExternalEvidenceRoot ("r2-sequence-" + (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ'))
...
try {
    ...
    foreach ($branch in $branches) {
        ...
        "Pithos compression R2 branch: $branch" | Set-Content -LiteralPath $currentLog -Encoding UTF8
        ...
        Invoke-NativeStep -Label 'commit branch evidence' -Log $currentLog -Action { & git commit -m "test: benchmark compression R2 $branch against 7-Zip" }
    ...
    $summaryCsv = Join-Path $sessionRoot 'R2_SEQUENCE_SUMMARY.csv'; $sequenceRows | Export-Csv -LiteralPath $summaryCsv -NoTypeInformation -Encoding UTF8
    $summaryTxt = Join-Path $sessionRoot 'R2_SEQUENCE_SUMMARY.txt'
    @(
        ...
        'winzip=INTENTIONALLY_NOT_USED',
        'status=PASS','',
        ($sequenceRows | Format-Table branch,pithos_archive_bytes,sevenzip_archive_bytes,delta_bytes,pithos_compress_ms,sevenzip_compress_ms,pithos_decompress_ms,sevenzip_decompress_ms -AutoSize | Out-String)
    ) | Set-Content -LiteralPath $summaryTxt -Encoding UTF8
    Write-Host "`nALL $expectedBranches COMPRESSION R2 BRANCHES PASSED" -ForegroundColor Green
    Write-Host "Summary: $summaryCsv"
    exit 0
}
catch {
    $failure = Join-Path $sessionRoot 'R2_SEQUENCE_FAILURE.txt'
    @(
        ...
        'STOP: later R2 branches were NOT tested.',
        'Do not modify source. Return this file and the branch log.'
    ) | Set-Content -LiteralPath $failure -Encoding UTF8
    Write-Error "R2 STACK STOPPED on $currentBranch : $($_.Exception.Message). Later branches were NOT tested. Evidence: $failure"
    exit 1
}

...
use pithos_format::{
    CODEC_FLAG_REQUIRED, CentralIndexRecord, EntryKind, EntryRecord, FOOTER_LEN, Footer,
    GlobalHeader, GroupRecord, GroupTableRecord, HEADER_LEN, IntegrityRecord,
    REQUIRED_COMPRESSED_SECTIONS, RestoreMapRecord, SECTION_ENTRY_LEN, SectionDirectoryRecord,
    SectionType,
};
...
fn choose_native_or_standard(
    input: &[u8],
    member_lengths: &[u64],
    standard: EncodedChoice,
    ...
    let (native_payload, _) = encode_exact_dedup(input, member_lengths, level)?;
    if native_payload.len() < standard.payload.len() {
        Ok(EncodedChoice {
...
fn encode_standard(
    ...
    codec_for_id(selected.codec).encode(
        input,
        &CodecConfig {
    ...
    Ok(EncodedChoice {
        chain_id: u32::from(selected.codec as u16) + 1,
        codec_id: selected.codec as u16,
        codec_version: codec_for_id(selected.codec).version(),
        level: selected.level,
        payload,
    })
}

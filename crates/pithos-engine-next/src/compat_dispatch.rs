...
/// PAF RAW archives are still owned by the hardened legacy reader.
///
/// The native-aware reader exists only because codec ids beyond the frozen
/// legacy registry need an extended decoder. Keeping RAW on the legacy path
/// preserves Gate A semantics (overlap detection, aggregate metadata budgets,
/// payload-independent metadata listing, exact error classes) instead of
/// partially reimplementing those guarantees in every experimental branch.

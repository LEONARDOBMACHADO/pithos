...
//!
//! The legacy engine remains the compatibility baseline. This crate re-exports
//! its public API and overrides only operations whose implementation has been
//! deliberately advanced in the stacked benchmark branches.
...
///
/// The baseline engine decoded the owning solid group once per file entry. A
/// 512 MiB archive-max group containing many files could therefore be decoded
/// repeatedly. This implementation validates the metadata/root first, then
/// decodes each group exactly once and fans the restored slices out to entries.

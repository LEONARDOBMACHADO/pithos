...
pub fn encode_exact_dedup(input: &[u8], member_lengths: &[u64], level: i32) -> Result<(Vec<u8>, NativeStats)> {
    ...
    for (id, bytes) in canonical.iter().enumerate() {
        ...
        records.push(CanonicalRecord { len: bytes.len() as u32, tokens });
...
pub fn decode_exact_dedup(payload: &[u8], expected_len: u64) -> Result<Vec<u8>> {
    ...
    for _ in 0..sequence_count { sequence.push(read_u32(&repr, pos)?); pos += 4; }
    let mut canonical = Vec::<Vec<u8>>::with_capacity(canonical_count);
    for _ in 0..canonical_count {
        let len = read_u32(&repr, pos)? as usize; pos += 4;
        let token_count = read_u32(&repr, pos)? as usize; pos += 4;
        let mut chunk = Vec::with_capacity(len);
        for _ in 0..token_count {
            let kind = *repr.get(pos).ok_or(PithosError::InvalidRange)?; pos += 1;
            match kind {
                0 => {
                    let literal_len = read_u32(&repr, pos)? as usize; pos += 4;
                    let end = pos.checked_add(literal_len).ok_or(PithosError::IntegerOverflow)?;
                    chunk.extend_from_slice(repr.get(pos..end).ok_or(PithosError::InvalidRange)?); pos = end;
                }
                1 => {
                    let base = read_u32(&repr, pos)? as usize; pos += 4;
                    let offset = read_u32(&repr, pos)? as usize; pos += 4;
                    let copy_len = read_u32(&repr, pos)? as usize; pos += 4;
                    let source = canonical.get(base).ok_or(PithosError::InvalidMetadata("reference base"))?;
                    let end = offset.checked_add(copy_len).ok_or(PithosError::IntegerOverflow)?;
                    chunk.extend_from_slice(source.get(offset..end).ok_or(PithosError::InvalidRange)?);
                }
                _ => return Err(PithosError::InvalidMetadata("reference token")),
            }
            if chunk.len() > len { return Err(PithosError::ResourceLimit("reference chunk output")); }
        }
        if chunk.len() != len { return Err(PithosError::InvalidRange); }
        canonical.push(chunk);
    }
    if pos != repr.len() { return Err(PithosError::InvalidMetadata("native trailing bytes")); }
    let mut out = Vec::with_capacity(expected_len as usize);
    for id in sequence {
        out.extend_from_slice(canonical.get(id as usize).ok_or(PithosError::InvalidMetadata("native reference"))?);
...
fn canonical_chunks(input: &[u8], member_lengths: &[u64]) -> Result<(Vec<Vec<u8>>, Vec<u32>, u64)> {
    ...
    for (member_id, len) in member_lengths.iter().copied().enumerate() {
        ...
        for draft in chunk_fastcdc(member, ChunkOrigin { entry_id: member_id as u64, object_id: 0, base_offset: base }, &cfg)? {
            ...
            let end = start.checked_add(draft.length as usize).ok_or(PithosError::IntegerOverflow)?;
            let bytes = input.get(start..end).ok_or(PithosError::InvalidRange)?;
            let hash = *blake3::hash(bytes).as_bytes();
            let found = by_hash.get(&hash).and_then(|ids| ids.iter().copied().find(|id| canonical[*id as usize].as_slice() == bytes));
            let id = match found {
                Some(id) => { duplicate = duplicate.checked_add(bytes.len() as u64).ok_or(PithosError::IntegerOverflow)?; id }
                None => { let id = canonical.len() as u32; canonical.push(bytes.to_vec()); by_hash.entry(hash).or_default().push(id); id }
            };
            sequence.push(id);
        }
...
fn reference_tokens(bytes: &[u8], previous: &[Vec<u8>], index: &HashMap<u64, Vec<(u32,u32)>>) -> Result<Vec<Token>> {
    if bytes.len() < MIN_COPY || previous.is_empty() { return Ok(vec![Token::Literal(bytes.to_vec())]); }
        ...
        if pos + ANCHOR <= bytes.len() {
            ...
            if let Some(candidates) = index.get(&key) {
                for &(base_id, offset) in candidates.iter().rev().take(MAX_ANCHOR_CANDIDATES) {
                    ...
                    while pos + len < bytes.len() && source_offset + len < source.len() && bytes[pos+len] == source[source_offset+len] { len += 1; }
                    if len >= MIN_COPY && best.is_none_or(|(_,_,best_len)| len > best_len) { best = Some((base_id, offset, len)); }
                }
            }
        }
        if let Some((base, offset, len)) = best {
            if !literal.is_empty() { tokens.push(Token::Literal(std::mem::take(&mut literal))); }
            tokens.push(Token::Copy { base, offset, len: len as u32 });
            pos += len;
        } else {
            literal.push(bytes[pos]); pos += 1;
        }
    }
    if !literal.is_empty() { tokens.push(Token::Literal(literal)); }
    let encoded_cost = tokens.iter().map(|t| match t { Token::Literal(v) => 5 + v.len(), Token::Copy{..} => 13 }).sum::<usize>() + 8;
    if encoded_cost >= bytes.len() + 8 { Ok(vec![Token::Literal(bytes.to_vec())]) } else { Ok(tokens) }
}

fn index_anchors(id: u32, bytes: &[u8], index: &mut HashMap<u64, Vec<(u32,u32)>>) {
    if bytes.len() < ANCHOR { return; }
    for offset in (0..=bytes.len()-ANCHOR).step_by(8) {
        ...
        if bucket.len() > MAX_ANCHOR_CANDIDATES * 2 { bucket.remove(0); }
    }
}

fn encode_record(record: CanonicalRecord, out: &mut Vec<u8>) -> Result<()> {
    out.extend_from_slice(&record.len.to_le_bytes()); out.extend_from_slice(&(record.tokens.len() as u32).to_le_bytes());
    for token in record.tokens {
        match token {
            Token::Literal(bytes) => { out.push(0); out.extend_from_slice(&(bytes.len() as u32).to_le_bytes()); out.extend_from_slice(&bytes); }
            Token::Copy { base, offset, len } => { out.push(1); out.extend_from_slice(&base.to_le_bytes()); out.extend_from_slice(&offset.to_le_bytes()); out.extend_from_slice(&len.to_le_bytes()); }
        }
    }
    Ok(())
}

fn validate_members(input:&[u8], members:&[u64])->Result<()> { let total=members.iter().try_fold(0u64,|a,b|a.checked_add(*b).ok_or(PithosError::IntegerOverflow))?; if total!=input.len() as u64 {Err(PithosError::InvalidMetadata("native member boundaries"))} else {Ok(())} }
...
#[cfg(test)]
mod tests { use super::*; #[test] fn moved_regions_roundtrip(){ let base=(0..200_000).map(|i|(i%251)as u8).collect::<Vec<_>>(); let mut changed=vec![7u8;1000]; changed.extend_from_slice(&base[20_000..180_000]); changed.extend_from_slice(&[8u8;2000]); let mut input=base.clone(); input.extend_from_slice(&changed); let (e,_)=encode_exact_dedup(&input,&[base.len()as u64,changed.len()as u64],5).unwrap(); assert_eq!(decode_exact_dedup(&e,input.len()as u64).unwrap(),input); } }

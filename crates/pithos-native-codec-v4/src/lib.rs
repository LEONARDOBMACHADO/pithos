...
pub fn encode_exact_dedup(
    ...
    let (baseline, base_stats) =
        pithos_native_v3::encode_exact_dedup(input, member_lengths, level)?;
    ...
    for len in member_lengths {
        ...
        metas.push(MemberMeta {
            original_len: *len,
            normalized_len: bytes.len() as u64,
            events,
        });
    ...
    let (nested, nested_stats) =
        pithos_native_v3::encode_exact_dedup(&normalized, &normalized_lengths, level)?;
...
fn canonicalize_member(input: &[u8]) -> Result<(Vec<u8>, Vec<Event>)> {
    ...
    let json = matches!(first, Some(b'{') | Some(b'['))
        ...
        if json {
            ...
            if byte.is_ascii_whitespace() {
                ...
                events.push(Event {
                    ...
                    bytes: input[start..index].to_vec(),
                });
                continue;
            }
            out.push(byte);
...
fn restore_members(normalized: &[u8], metas: &[MemberMeta], expected_len: u64) -> Result<Vec<u8>> {
    ...
    for meta in metas {
        ...
        let end = base
            .checked_add(meta.normalized_len as usize)
            .ok_or(PithosError::IntegerOverflow)?;
        let member = normalized.get(base..end).ok_or(PithosError::InvalidRange)?;
        let mut cursor = 0_usize;
        for event in &meta.events {
            let offset = event.offset as usize;
            if offset < cursor || offset > member.len() {
                return Err(PithosError::InvalidMetadata("canonical event order"));
            }
            result.extend_from_slice(&member[cursor..offset]);
            result.extend_from_slice(&event.bytes);
            cursor = offset;
        }
        result.extend_from_slice(&member[cursor..]);
        if result.len() - member_start != meta.original_len as usize {
            return Err(PithosError::InvalidMetadata("canonical original length"));
        }
        if result.len() as u64 > expected_len {
            return Err(PithosError::ResourceLimit("canonical restore output"));
        }
        base = end;
    }
    if base != normalized.len() || result.len() as u64 != expected_len {
        return Err(PithosError::InvalidRange);
    }
    Ok(result)
}
...
fn encode_metadata(metas: &[MemberMeta]) -> Result<Vec<u8>> {
    ...
    for meta in metas {
        ...
        out.extend_from_slice(&meta.normalized_len.to_le_bytes());
        out.extend_from_slice(&(meta.events.len() as u32).to_le_bytes());
        for event in &meta.events {
            out.extend_from_slice(&event.offset.to_le_bytes());
            out.extend_from_slice(&(event.bytes.len() as u32).to_le_bytes());
            out.extend_from_slice(&event.bytes);
        }
    }
    Ok(out)
}
fn decode_metadata(data: &[u8], expected: usize) -> Result<Vec<MemberMeta>> {
    let count = read_u32(data, 0)? as usize;
    if count != expected {
        return Err(PithosError::InvalidMetadata("canonical member count"));
    }
    let mut pos = 4;
    ...
    for _ in 0..count {
        ...
        pos += 8;
        let normalized_len = read_u64(data, pos)?;
        pos += 8;
        let ec = read_u32(data, pos)? as usize;
        pos += 4;
        let mut events = Vec::with_capacity(ec);
        for _ in 0..ec {
    ...
    if pos != data.len() {
        return Err(PithosError::InvalidMetadata("canonical metadata trailing"));
    }
    Ok(metas)
}
fn convert_stats(s: pithos_native_v3::NativeStats, encoded: u64) -> NativeStats {
    NativeStats {
...
mod tests {
    use super::*;
    #[test]
    fn json_whitespace_is_reversible() {
        let a = b"{ \r\n  \"a\" : 1, \"b\" : [1, 2] }";
        let input = [a.as_slice(), a.as_slice()].concat();
        let (e, _) = encode_exact_dedup(&input, &[a.len() as u64, a.len() as u64], 5).unwrap();

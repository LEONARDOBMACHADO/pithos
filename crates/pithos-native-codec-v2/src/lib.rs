...
pub fn encode_exact_dedup(
    ...
    if member_lengths.iter().try_fold(0_u64, |a, b| {
    ...
    for (member_id, len) in member_lengths.iter().copied().enumerate() {
        ...
        for draft in chunk_fastcdc(
            ...
            let existing = by_hash.get(&hash).and_then(|ids| {
                ids.iter()
                    .copied()
...
pub fn decode_exact_dedup(payload: &[u8], expected_len: u64) -> Result<Vec<u8>> {
    ...
    for _ in 0..seq_count {
        seq.push(read_u32(&repr, pos)?);
        pos += 4;
    }
    let mut canonical = Vec::<Vec<u8>>::with_capacity(canon_count);
    for _ in 0..canon_count {
        canonical.push(decode_record(&repr, &mut pos, &canonical)?);
    }
    if pos != repr.len() {
...
fn decode_record(data: &[u8], pos: &mut usize, canonical: &[Vec<u8>]) -> Result<Vec<u8>> {
    let kind = *data.get(*pos).ok_or(PithosError::InvalidRange)?;
    *pos += 1;
    ...
    *pos += 4;
    match kind {
        ...
        1 => {
            let base_id = read_u32(data, *pos)? as usize;
            *pos += 4;
            let count = read_u32(data, *pos)? as usize;
            *pos += 4;
            let mut out = canonical
                .get(base_id)
                .ok_or(PithosError::InvalidMetadata("delta base"))?
                .clone();
            if out.len() != len {
                return Err(PithosError::InvalidMetadata("delta length"));
            }
            for _ in 0..count {
                let off = read_u32(data, *pos)? as usize;
                *pos += 4;
                let value = *data.get(*pos).ok_or(PithosError::InvalidRange)?;
                *pos += 1;
                *out.get_mut(off).ok_or(PithosError::InvalidRange)? = value;
            }
            Ok(out)
        }
        2 => {
            let base_id = read_u32(data, *pos)? as usize;
            *pos += 4;
            let prefix = read_u32(data, *pos)? as usize;
            *pos += 4;
            let suffix = read_u32(data, *pos)? as usize;
            *pos += 4;
            let middle_len = read_u32(data, *pos)? as usize;
            *pos += 4;
            let end = pos
                .checked_add(middle_len)
                .ok_or(PithosError::IntegerOverflow)?;
            let middle = data.get(*pos..end).ok_or(PithosError::InvalidRange)?;
            *pos = end;
            let base = canonical
                .get(base_id)
                .ok_or(PithosError::InvalidMetadata("delta base"))?;
            if prefix + suffix > base.len() || prefix + middle_len + suffix != len {
                return Err(PithosError::InvalidMetadata("splice layout"));
            }

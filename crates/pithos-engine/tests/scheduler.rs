...
fn scheduler_honors_dag_priority_and_returns_deterministic_spool_order() {
    ...
    let tasks = vec![
        ScheduledTask::new(30, JobPriority::Benchmark, vec![], estimate(1), move |_| {
    ...
    assert_eq!(completed.iter().map(|item| item.task_id).collect::<Vec<_>>(), [10, 20, 30]);
...
fn scheduler_enforces_weighted_memory_budget() {
    ...
    let make_task = |id| {
        ...
        ScheduledTask::new(id, JobPriority::PackForeground, vec![], estimate(8), move |_| {
...
fn scheduler_rejects_cycles_oversized_jobs_and_observes_cancellation() {
    ...
    let config = SchedulerConfig::new(2, 8, directory.path()).unwrap();
    let cycle = vec![
        ScheduledTask::new(1, JobPriority::Benchmark, vec![2], estimate(1), |_| Ok(vec![])),
        ScheduledTask::new(2, JobPriority::Benchmark, vec![1], estimate(1), |_| Ok(vec![])),
    ];
    assert!(matches!(
...
fn scheduler_queue_capacity_is_bounded_from_worker_count() {
    let directory = tempfile::tempdir().unwrap();
    assert_eq!(SchedulerConfig::new(1, 1, directory.path()).unwrap().queue_capacity(), 4);
    assert_eq!(SchedulerConfig::new(4, 1, directory.path()).unwrap().queue_capacity(), 8);
}

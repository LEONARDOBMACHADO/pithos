...
- **Decisão:** O produto chama-se **Pithos R1**. A expressão "Pithos R1 / R3" está proibida. O formato binário é **PAF 0.1-draft** durante o desenvolvimento e será congelado como **PAF 1.0** no lançamento. A extensão pública definitiva é **`.pits`**, escolhida por associação visual/fonética direta com *Pithos* e por não ter sido encontrada uma associação de formato de arquivo estabelecida relevante durante a revisão realizada em agosto de 2026. `.pts` foi rejeitada como extensão pública por colisões existentes; `.phs` foi usada apenas provisoriamente no branch de desenvolvimento. `.phs`, `.pts` e `.pithos` permanecem legíveis durante desenvolvimento/transição, mas novos archives são gerados somente como `.pits`. A identificação e validação do formato são feitas pelo magic/layout PAF e nunca pela extensão do filename.

---

## ADR-002: Seekable Pack Output
- **Status:** Aceito
- **Contexto:** Geração do arquivo `.pits` exige escrita de offsets globais, CentralIndex e BLAKE3 root no fechamento.
...
- **Status:** Aceito; Gate C3 CLOSED / PASS
